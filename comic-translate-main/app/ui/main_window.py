...
# Polices par défaut à proposer (ajoutez ici les fichiers ttf/otf/woff téléchargés)
default_fonts = [
    'Avenue de Madison.ttf',
    'ShadeBlue-2OozX.ttf',
    ...
    'ComicNeueSansID.ttf',
    'AVENGEANCE MIGHTIEST AVENGER.ttf',
    'animeace2_ital.ttf',
    'animeace2_bld.ttf',
    'animeace2_reg.ttf',
    'KOMIKAX_.ttf',
...
class ComicTranslateUI(QtWidgets.QMainWindow):
    ...
    def _create_main_content(self):
        ...
        # Charger les polices par défaut si elles existent dans le dossier fonts
        for font_file in default_fonts:
            font_path = os.path.join(font_folder_path, font_file)
            if os.path.exists(font_path):
                self.add_custom_font(font_path)
        # Charger aussi toutes les polices déjà présentes dans le dossier fonts
        ...
        # Synchroniser la combo box avec la police globale APRÈS création
        settings = QSettings("ComicLabs", "ComicTranslate")
        settings.beginGroup('text_rendering')
        font_family = settings.value('font_family', None)
        settings.endGroup()
        if font_family:
    ...
    def show_settings_page(self):
        if not self.settings_page:
            self.settings_page = SettingsPage(self)
            # Connexion du signal de changement de police
            if hasattr(self.settings_page.ui, 'font_combo_box'):
                self.settings_page.ui.font_combo_box.currentTextChanged.connect(self.set_font)
        # Synchronise la combo box avec la police globale à chaque ouverture des settings
        settings = QSettings("ComicLabs", "ComicTranslate")
        settings.beginGroup('text_rendering')
        font_family = settings.value('font_family', None)
        settings.endGroup()
    ...
    def set_font(self, font_family: str):
        ...
        # Synchronise la combo box principale avec la police globale
        if self.font_dropdown.currentText() != font_family:
            index = self.font_dropdown.findText(font_family)
            if index != -1:
