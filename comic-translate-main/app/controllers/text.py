...
class TextController:
    ...
    def on_blk_rendered(self, text: str, font_size: int, blk: TextBlock):
        ...
        # --- Correction : forcer la police globale des settings ---
        from PySide6.QtCore import QSettings
        settings = QSettings("ComicLabs", "ComicTranslate")
        settings.beginGroup('text_rendering')
        font_family_setting = settings.value('font_family', None)
        settings.endGroup()
        font_family = font_family_setting if font_family_setting else render_settings.font_family
        # ----------------------------------------------------------
    ...
    def render_text(self):
        if self.main.image_viewer.hasPhoto() and self.main.blk_list:
            ...
            font_family_setting = settings.value('font_family', None)
            settings.endGroup()
            font_family = font_family_setting if font_family_setting else self.main.font_dropdown.currentText()
            # ----------------------------------------------------------
            outline_width = float(self.main.outline_width_dropdown.currentText())
